...
def add_vortex():
    ...
    bpy.context.object.location = (0, -2.5, -2)
...
if __name__ == "__main__":
    ...
    # roomを追加
    append_collection(os.path.expanduser("~/spike/45_sagyoudai/parts/room.blend"), "room")
    # 正面の壁を非表示に
    bpy.context.scene.objects["f_wall"].hide_viewport = True
    bpy.context.scene.objects["f_wall"].hide_render = False

    # 既存のカメラとライトを削除する
    for obj in [o for o in bpy.data.objects if o.name in ['Camera', 'Light']]:
        bpy.data.objects.remove(obj)
    # 実験ルーム用のカメラとライトを配置する
    append_collection(os.path.expanduser("~/spike/45_sagyoudai/parts/camera_and_lights.blend"), "camera_and_lights")
    ...
    # テキスト追加
    txt_obj = add_text("ハロー!!", 2, 0.2, font, material)
